...
export default function Navbar({ onMenuClick }: { onMenuClick?: () => void }) {
  const { activeSection, handleNavClick, navRef } = useActiveSection(NAV_LINKS.map((l) => l.href));

  const isWide = useIsWide();

  return (
    <div ref={navRef} className={`${isWide ? "lg:sticky top-6" : "fixed top-1"} z-50 flex justify-center w-full font-manrope px-4 left-0`}>
